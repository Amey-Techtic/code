...
exports.productCheckout =async (req, res) => {
   return res.status(200).json({status:true, message: "This is product checkout request"});
}
...
exports.addProducts = async(req, res) => {
   try {
      ...
      if(productAdded){
         ...
         return res.status(200).json({message: "Unable to add product please enter the valid product details!", success: false});
      }
   } catch (error) {
...
exports.getProducts = async(req, res) => {
   try {
      ...
      if(productData){
         res.status(200).json({productData: productData?.productData, pagination: productData?.Pagination, success: true});
      }
      else{
